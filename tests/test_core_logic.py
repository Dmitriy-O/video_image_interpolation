...
def make_moving_frames(n_frames=40, h=32, w=48, slow_speed=1, fast_speed=5):
    ...
    З *ідеальними* (hand-crafted) мітками slow/fast при використанні 'hold' на швидкій фазі
    часто видно помітний позитивний ΔPSNR (див. також діагностичні запуски в історії розмови).
    Це найкращий спосіб показати «сильний» ефект для курсової/демо.

    Щоб отримати реальний відеофайл — можна легко розширити функцію (cv2.VideoWriter) або
    використати matplotlib animation + imageio / opencv для експорту послідовності в .mp4.
    """
    frames = []
